...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    public static final String alamat_h = "alamat_halte";
    ...
    private String url = "http://192.168.43.51/Abusway/Masyarakat/mkoridor.php?id_rute=";
            ...
            public void onResponse(String response) {
                ...
                try {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        ...
                        alamat = jsonObject.getString(alamat_h);
                        ketera = jsonObject.getString(ket);
                        latLng = new LatLng(Double.parseDouble(jsonObject.getString(LAT)), Double.parseDouble(jsonObject.getString(LNG)));

                        // Menambah data marker untuk di tampilkan ke google map
                       // addMarker(latLng, title);
                        gMap.addMarker(new MarkerOptions().position(latLng).title(title).snippet("Alamat : "+alamat).icon(BitmapDescriptorFactory.fromResource(R.drawable.icmm)));
